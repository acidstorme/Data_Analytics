...
if __name__ == "__main__":
    try:
        ...
        row = execute_single_row_query(session, "SELECT COUNT(*) As Count FROM e_commerce.Product;")
        print("No of Products in database are : ", row.count)
        
        #Getting Count of Customers in the database
        row = execute_single_row_query(session, "SELECT COUNT(*) As Count FROM e_commerce.Customer;")
        print("No of Customer in database are : ", row.count)
        
        #Getting count of product wise likes by customer
        rows = execute_query(session, "SELECT liked_prdt_id, title, Count(*) as Likes FROM e_commerce.Product_Liked_By_Customer GROUP BY liked_prdt_id;")
