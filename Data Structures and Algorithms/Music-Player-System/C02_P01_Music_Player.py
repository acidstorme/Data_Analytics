...
class LinkedList:
    ...
    def delete_song(self, song_name):
        ...
        while self.head_node != None:
            ...
            while(temp_node.song.song_name!=song_name):
                ...
                temp_node = temp_node.next
                if temp_node == None:
                    print("Sorry! Song not found in playlist\n")
                    return

            if temp_node.song.song_name==song_name: #delete node
    ...
    def sort_list(self):
        ...
        if self.head_node != None:
            while(1):
                ...
                tmp = self.head_node
                while(tmp.next != None):
                    if tmp.song.song_name > tmp.next.song.song_name:
                        # swap them
                        swap_count += 1
                        p = tmp.song
                        tmp.song = tmp.next.song
                        tmp.next.song = p
                        tmp = tmp.next
                    else:
                        tmp = tmp.next
    ...
    def shuffle_song(self):
        ...
        if self.head_node != None:
            songindex = random.randint(0, self.count-1)
            ctr = 0
            tmp = self.head_node
            while(ctr != self.count):
                if ctr == songindex:
                    return tmp.song.song_name
                else:
                    tmp=tmp.next
                    ctr+=1
